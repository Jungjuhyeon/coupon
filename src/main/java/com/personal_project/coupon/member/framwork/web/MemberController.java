...
public class MemberController {
    ...
    @PostMapping("/signup")
    public SuccessResponse<MemberOutPutDTO> signUp(@RequestBody MemberInfoDTO request){
        Member member = authMember.signUp(request);
        return SuccessResponse.success(MemberOutPutDTO.mapToDTO(member));
    }

    @PostMapping("login")
    public SuccessResponse<MemberOutPutDTO> login(@RequestBody MemberLoginDTO request){
